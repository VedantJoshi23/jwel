...
| Payments | `modules/payments` | `/api/v1/payments/webhook/razorpay`, `/api/v1/payments/verify` |
...
**Closed in Milestone 12**: `PaymentProviderPort.refund` now exists and
`PaymentsService.refundForOrder` calls Razorpay's Refunds API before marking
the `Payment` row `REFUNDED`. That ordering is deliberate — money first,
bookkeeping second — so a gateway failure leaves the row untouched and the
return retryable, rather than recording a refund the customer never received.
`ReturnsService` also refunds *before* restocking for the same reason: a
restock-first failure would credit inventory for a refund that never happened,
and a retry would restock the same unit twice.
...
| Refund API integration | **Closed in Milestone 12** — `PaymentProviderPort.refund` calls Razorpay's Refunds API; see §3.11 for the money-before-bookkeeping ordering |
| Razorpay adapter | **Closed in Milestone 12** — `RazorpayPaymentProvider` is the sole adapter (ADR-0005); the Stripe adapter is deleted and the port's return shape is now `{ providerRef, checkout: { keyId, orderId, simulated } }` |
